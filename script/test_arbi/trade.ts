...
async function main() {
  ...
  console.log("margin:", margin, margin2);
    ...
    // amount = await arbiMuncher.connect(deployer).tradeGmxToUni(usdcAddress, wethAddress, usdcBalance);
    // await amount.wait();
    // console.log(amount);

    amount = await arbiMuncher.connect(deployer).tradeUniToGmx(usdcAddress, wethAddress, usdcBalance);
    await amount.wait();
    console.log(amount);
